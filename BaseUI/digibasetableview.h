#ifndef DIGIBASETABLEVIEW_H
#define DIGIBASETABLEVIEW_H

/**
 * @brief 客户端公共库中的基础表格 TableView 封装类
...
class DigiBaseTableView : public QTableView, public IWidgetBase, public IStyleSheetBase
{
    Q_OBJECT

public:
    explicit DigiBaseTableView(QWidget *parent = nullptr);

    void SetRowHeight(qreal height);              //每行的高度基值
    void SetHeaderHeight(qreal height);           //横表头高度
    void SetBGColor(const QColor &color);         //tableView背景颜色
    ...
    void SetBorderRadius(int radius);//设置列宽
...
private:
    void InitStyle();
    void InitNoDataWidget();
    void SetNoDataWidgetVisible(bool visible);
...
private:
    ...
    QString m_BGColorHex;          //TableView背景颜色
    ...
    QString m_HoverColorHex;       //悬停背景颜色
    ...
    int m_BorderRadius;   //边框半径

    QVBoxLayout *m_NoDataLayout;           //无数据时的提示布局
    DigiBaseLabel *m_NoDataTextLab;    //无数据时的提示文字
    DigiBaseLabel *m_NoDataPictureLab; //无数据时的提示图片
};

#endif // DigiBaseTableView_H
